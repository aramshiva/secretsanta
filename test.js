...
sendMatchingSlackMessage();
async function sendMatchingSlackMessage() {
    ...
    const slackToken = await question("Enter Slack Token: ");
    const userSlackId = await question("Enter User Slack ID: ");
    const userName = await question("Enter User Name: ");
    const matchName = await question("Enter Match Name: ");
    const addressLine1 = await question("Enter Address Line 1: ");
    const addressLine2 = await question("Enter Address Line 2 (optional): ");
    const city = await question("Enter City: ");
    const state = await question("Enter State: ");
    const postal = await question("Enter Postal Code: ");
    const country = await question("Enter Country: ");
    const likes = await question("Enter Likes: ");
    const dislikes = await question("Enter Dislikes: ");

    rl.close();

    const message = `Hey ${userName}, you've been matched for Secret Santa!\n\n` +
